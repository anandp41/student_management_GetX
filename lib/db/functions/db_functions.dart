...
Future<void> getAllStudents() async {
  ...
  DbListController dbList = Get.find<DbListController>();
  final _values = await _db.rawQuery('SELECT * FROM student');

  dbList.clearStudentList();
  //studentListNotifier.notifyListeners();
  for (var map in _values) {
    final student = StudentModel.fromMap(map);
    dbList.addToStudList(student);
    //studentListNotifier.notifyListeners();
  }
}
...
Future<void> deleteRow(String admno) async {
        ...
        Row(
          ...
          children: [
            ElevatedButton.icon(
              onPressed: () async {
                await _db
                    .rawDelete('DELETE from student where admno=?', [admno]);
                await getAllStudents();
                Get.back();
              },
              icon: const Icon(
